...
public class DevoirController implements Initializable {
    ...
    private ArrayList<Plat> platsChoisis = new ArrayList<>();
    Alert alert;
    @FXML
    private ListView lvCategories;
    @FXML
    private ListView lesEntrees;
    @FXML
    ...
    @FXML
    private TableView<Plat> lvEntrees;
    @FXML
    private TableView<Plat> lvDessert;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        lesEntrees = new ListView();// pour rajouter les entrees
    ...
    public void lvCategoriesClicked(Event event) {
        ...
        // Si je veux recuperer un categorieje prend get la chaine de caractere
        String categorie = (String) lvCategories.getSelectionModel().getSelectedItem();
        // On récupère la liste des plats de cette catégorie
        ArrayList<Plat> liste = lesPlats.get(categorie);

        String entrees =(String)lesEntrees.getId();
        ArrayList<Plat> plats = lesPlats.get(entrees);


// et la j'affiche les plats dans la categorie donc on reprend notre array list et je lui dit que je la recupere pour la mettre dans les plats

        // On ajoute les plats dans la TableView
        tvPlats.getItems().addAll(liste);
        lblCategorie.setText("Catégorie : " + categorie);
        lesEntrees.getItems().addAll(liste);


// j'aurais voulu que quand la personne appuie sur l'un des choix donc desserrt plat ou entree elle puisse l'avoir pour chacun, si ele appuie sur entree elle aentree si elle appuie sur dessert elle aura dessert et apres elle click sur ajouter
    ...
    public void btnAjouterClicked(Event event) {
        ...
        // A vous de jouer pour vérfiier les infos et remplir la HashMap "lesCartes"
            ...
            if (lvCategories.getSelectionModel().getSelectedItem() == null)
            {
                alert.setContentText("Attention sélectionner une catégorie ");
                alert.showAndWait();
            }
            else if (tvPlats.getSelectionModel().getSelectedItem() == null)
            {
                alert.setContentText("verifier que vous avez selectionner un plat");
                alert.showAndWait();
            }
            else if (lesEntrees.getSelectionModel().getSelectedItem() == null)
            {
                alert.setContentText("verifier que vous avez selectionner un entrees");
                alert.showAndWait();
            }
            else if (lvDessert.getSelectionModel().getSelectedItem() == null)
            {
                alert.setContentText("verifier que vous avez selectionner un entrees");
                alert.showAndWait();
            }
            else if (platsChoisis.size() >= 3)
            {
                alert.setContentText("Attention tu as déjà choisi 3 plats pour ce menu");
                alert.showAndWait();
